...
const isUsernameLengthValid: Validator<LoginForm> = data =>
  data.username.length >= 2 ? right(data) : left({ field: 'username', error: 'at least 2 characters' })

// password
const isPasswordLengthValid: Validator<LoginForm> = data =>
data.password.length >= 6 ? right(data) : left({ field: 'password', error: 'at least 6 characters' })

const isPasswordFormatValid: Validator<LoginForm> = data =>
  /[0-9]/g.test(data.password) ? right(data) : left({ field: 'password', error: 'at least one number' })
